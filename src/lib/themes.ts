 ...
 *
 * 【概要】
 * アプリ全体のカラーテーマ（Light/Dark/Dim）を定義。
 * カラーパレット、タイポグラフィ、形状などのデザイントークンを管理。
 *
 * 【テーマの種類】
 * - defaultTheme: ライトテーマ（明るい背景）
 * - darkTheme: ダークテーマ（暗い背景）
 * - dimTheme: ディムテーマ（薄暗い背景、目に優しい）
 *
 * 【ALFデザインシステムとの関係】
 * - src/alf/: 新しいデザインシステム（推奨）
 * - このファイル: レガシーテーマ定義（一部互換性のために残存）
 *
 * 【パレット構造】
 * - default: 基本色（背景、テキスト、ボーダー等）
 * - primary: プライマリ色（青系、アクション）
 * - secondary: セカンダリ色（緑系、成功）
 * - inverted: 反転色（ボタン背景等）
 * - error: エラー色（赤系、警告）
 *
 * 【Goユーザー向け補足】
 * - スプレッド演算子(...): Goの構造体埋め込みに類似
 *   既存のオブジェクトを展開して新しいオブジェクトを作成
 * - Theme型: GoのTheme structに相当するインターフェース
 */
