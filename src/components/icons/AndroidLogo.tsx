/**
 * Androidロゴアイコンモジュール
 * Android Logo Icon Module
 *
 * Androidオペレーティングシステムの公式ロゴアイコンを提供します。
 * Android特有の機能やAndroidアプリへのリンク、プラットフォーム識別などに
 * 使用されます。
 *
 * Provides the official logo icon for the Android operating system.
 * Used for Android-specific features, links to Android apps, or
 * platform identification.
 *
 * 【Goユーザー向け補足】
 * - ロゴアイコン: ブランドアイデンティティを表す公式デザイン
 *   商標権に注意が必要で、Googleのブランドガイドラインに従う必要がある
 * - viewBox="0 0 24 24": SVGの座標系を定義（Goのグラフィックスライブラリの
 *   ビューポート設定に相当）
 */

// SVGアイコンテンプレート - 単一パスSVGアイコンを作成するヘルパー関数
// SVG icon template - helper function to create single-path SVG icons
import {createSinglePathSVG} from './TEMPLATE'

/**
 * Androidロゴアイコン
 * Android Logo Icon
 *
 * Androidのマスコット「Bugdroid」のロゴデザインです。
 * 特徴的な緑色のロボット形状で、アンテナ、目、腕、脚を持ちます。
 *
 * Logo design of Android's mascot "Bugdroid".
 * Features a distinctive green robot shape with antennae, eyes, arms, and legs.
 *
 * @example
 * ```tsx
 * <AndroidLogo fill="#3DDC84" size={48} />
 * ```
 */
export const AndroidLogo = createSinglePathSVG({
  path: 'm15.604 2.47-.642 1.112a4.997 4.997 0 0 1 2.05 4.034H7.005c0-1.658.809-3.123 2.05-4.034L8.41 2.47a.313.313 0 0 1 .542-.313l.627 1.086a4.97 4.97 0 0 1 2.428-.63c.882 0 1.709.23 2.428.63l.627-1.085a.311.311 0 0 1 .426-.115c.151.087.202.278.115.428ZM9.506 5.114a.625.625 0 1 0 1.25 0 .625.625 0 0 0-1.25 0Zm3.753 0a.625.625 0 1 0 1.25 0 .625.625 0 0 0-1.25 0ZM3.878 8.866a1.251 1.251 0 0 1 2.501 0v5.004a1.252 1.252 0 0 1-2.501 0V8.866Zm13.759 0a1.251 1.251 0 0 1 2.501 0v5.004a1.251 1.251 0 0 1-2.501 0V8.866ZM7.005 17.622h1.25v3.128a1.251 1.251 0 0 0 2.502 0v-3.128h2.502v3.128a1.251 1.251 0 0 0 2.501 0v-3.128h1.25V8.241H7.006v9.381Z',
})
