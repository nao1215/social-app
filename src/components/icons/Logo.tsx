// React Native SVG - SVG要素とPath要素のレンダリング
import Svg, {Path} from 'react-native-svg'

// アイコン共通プロパティ - SVGアイコンの標準プロパティと共通フック
import {type Props, useCommonSVGProps} from './common'
// SVGテンプレート - 単一パスSVGアイコンを作成するユーティリティ
import {createSinglePathSVG} from './TEMPLATE'

/**
 * Blueskyのマークアイコン - アプリのシンボルマーク（蝶のような形状）
 * Bluesky mark icon - the app's symbol mark (butterfly-like shape)
 */
export const Mark = createSinglePathSVG({
  path: 'M6.335 4.212c2.293 1.76 4.76 5.327 5.665 7.241.906-1.914 3.372-5.482 5.665-7.241C19.319 2.942 22 1.96 22 5.086c0 .624-.35 5.244-.556 5.994-.713 2.608-3.315 3.273-5.629 2.87 4.045.704 5.074 3.035 2.852 5.366-4.22 4.426-6.066-1.111-6.54-2.53-.086-.26-.126-.382-.127-.278 0-.104-.041.018-.128.278-.473 1.419-2.318 6.956-6.539 2.53-2.222-2.331-1.193-4.662 2.852-5.366-2.314.403-4.916-.262-5.63-2.87C2.35 10.33 2 5.71 2 5.086c0-3.126 2.68-2.144 4.335-.874Z', // Blueskyマークの形状を定義するSVGパス
})

/**
 * Blueskyの完全なロゴ - マークとテキストを含むフルロゴ
 * Full Bluesky logo - complete logo including mark and text
 */
export function Full(
  props: Omit<Props, 'fill' | 'size' | 'height'> & {
    markFill?: Props['fill'] // マーク部分の塗りつぶし色
    textFill?: Props['fill'] // テキスト部分の塗りつぶし色
  },
) {
  const {fill, size, style, gradient, ...rest} = useCommonSVGProps(props) // 共通SVGプロパティを取得
  const ratio = 123 / 555 // ロゴのアスペクト比（高さ/幅）

  return (
    <Svg
      fill="none"                    // デフォルトの塗りつぶしなし
      {...rest}                      // その他のSVGプロパティを展開
      viewBox="0 0 555 123"          // SVGビューボックス（座標系）
      width={size}                   // 指定されたサイズでの幅
      height={size * ratio}          // アスペクト比に基づく高さ
      style={[style]}>               {/* カスタムスタイルを適用 */}
      {gradient} {/* グラデーション定義（ある場合） */}
      {/* Blueskyマーク部分のパス */}
      <Path
        fill={props.markFill ?? fill}  // マーク専用の色またはデフォルト色
        fillRule="evenodd"             // 塗りつぶしルール
        clipRule="evenodd"             // クリッピングルール
      ...
      {/* "bluesky"テキスト部分のパス */}
      <Path
        fill={props.textFill ?? fill}  // テキスト専用の色またはデフォルト色
        fillRule="evenodd"             // 塗りつぶしルール
        clipRule="evenodd"             // クリッピングルール
