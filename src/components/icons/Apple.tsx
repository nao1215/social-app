/**
 * リンゴアイコンモジュール
 * Apple (Fruit) Icon Module
 *
 * リンゴの果実を表すアイコンを提供します。
 * 食品関連の機能、健康・栄養関連のコンテンツ、または
 * アプリケーションのテーマとして使用されます。
 *
 * Provides an icon representing an apple fruit.
 * Used for food-related features, health/nutrition content, or
 * as an application theme.
 *
 * 【Goユーザー向け補足】
 * - このアイコンは果実のリンゴです（Apple Inc.のロゴとは異なります）
 * - fillRule/clipRule: SVGのパス描画ルールを定義
 *   "evenodd"は複雑な形状の塗りつぶし方法を指定（交差判定アルゴリズム）
 */
...
/**
 * リンゴアイコン (ストローク2, 角丸なし)
 * Apple Icon (Stroke 2, No Corner Radius)
 *
 * リンゴの果実を表現したアイコンです。
 * 葉と茎を含む自然なリンゴの形状をしています。
 * ストローク幅2px、角の丸みなしのクリアなスタイルを使用しています。
 *
 * Icon representing an apple fruit.
 * Features a natural apple shape including a leaf and stem.
 * Uses clear styling with 2px stroke width and no corner radius.
 *
 * @example
 * ```tsx
 * <Apple_Stroke2_Corner0_Rounded fill="red" size={32} />
 * ```
 */
